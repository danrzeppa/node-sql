...
Postgres.prototype.visit = function(node) {
  switch(node.type) {
    ...
    case 'UNION'           : return this.visitUnion(node);
...
Postgres.prototype.visitFrom = function(from) {
  ...
  if (from.alias) result = result.concat('AS ' + this.quote(from.alias));
  return result;
};
...
Postgres.prototype.visitUnion = function(union) {
  var result = [];
  //this._visitingJoin = true;
  result='('
  result = result.concat(this.visit(union.leftSide));
  result = result.concat(' UNION ');
  result = result.concat(this.visit(union.rightSide));
